...
api_map <- dmdfile$bnf_full_final
...
The `practice_wise()` function require following three parameters, 

- Combined NHS prescription dataset, genereated using `csv2dat()` function
- A `data.frame` containing GP Practices
- BNF Code mapped file, genereated using `importdmd()` function 
...
practice_wise(data201812, UK_GP2018, api_map)
```
